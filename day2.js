...
const partOne = () => {
    ...
    for ( let i = 0; i < data.length; i++ ) {
        ...
        for ( let j = 0; j < str.length; j++ ) {
            val === str[j] ? count++ : null;
        }
        count >= min && count <= max ? tally++ : null;
    }
...
const partTwo = () => {
    let tally = 0;
    for ( let i = 0; i < data.length; i++ ) {
        const pos1 = data[i][0] - 1;
        const pos2 = data[i][1] - 1;
        const val = data[i][2];
        const str = data[i][3];
        let count = 0;
        str[pos1] === val ? count++ : null;
        str[pos2] === val ? count++ : null;
        count === 1 ? tally++ : null;
    }
    console.log( "Part Two Solution: " + tally );
}
