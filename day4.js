...
const partOne = () => {
    ...
    for ( let i = 0; i < newArray.length; i++ ) {
        let tempData = newArray[i].split( " " );
        let newTempData = [];
        for ( let j = 0; j < tempData.length; j++ ) {
            tempData[j].length > 0 ? newTempData.push( tempData[j] ) : null;
        }
        newArray[i] = newTempData;
    }
    for (let i = 0; i < newArray.length; i++ ) {
        newArray[i].length === 8 ? tally++ : null;
        if ( newArray[i].length === 7 ) {
            let x = 0;
            for (let j = 0; j < newArray[i].length; j++ ) {
                newArray[i][j].includes( "cid" ) ? x++ : null;
            }
            !x ? tally++ : null;
        }
    }
    console.log( "Part One Solution: " + tally );
    partTwo( newArray );
}
...
const partTwo = ( arr ) => {
    ...
    for ( let i = 0; i < arr.length; i++ ){
        let invalid = 0;
        let potential = 0;
        arr[i].length < 7 ? invalid++ : null;
        for ( let j = 0; j < arr[i].length; j++) {
            let test = arr[i][j];
            if ( test.includes( "byr" ) ) {
                let byr = test.split( ":" );
                parseInt( byr[1]) >= 1920 && parseInt( byr[1] ) <= 2002 ? potential++ : invalid++ ;
            }
            if ( test.includes( "iyr" ) ) {
                let iyr = test.split( ":" );
                parseInt( iyr[1] ) >= 2010 && parseInt( iyr[1] ) <= 2020 ? potential++ : invalid++ ;
            }
            if ( test.includes( "eyr" ) ) {
                let eyr = test.split( ":" );
                parseInt( eyr[1] ) >= 2020 && parseInt( eyr[1] ) <= 2030 ? potential++ : invalid++ ;
            }
            if ( test.includes( "hgt" ) ) {
                let hgt = test.split( ":" );
                if ( hgt[1].includes( "cm" ) ) {
                    let height = hgt[1].split( "cm" );
                    parseInt( height[0] ) >= 150 && parseInt( height[0] ) <= 193 ? potential++ : invalid++ ;
                }
                if ( hgt[1].includes( "in" ) ) {
                    let height = hgt[1].split( "in" );
                    parseInt( height[0] ) >= 59 && parseInt( height[0] ) <= 76 ? potential++ : invalid++ ;
                }
            }
            if ( test.includes( "hcl" ) ) {
                let hcl = test.split( ":" );
                let x = (/^#[0-9A-F]{6}/i).test( hcl[1] );
                x ? potential++ : invalid++ ;
            }
            if ( test.includes( "ecl" ) ) {
                let colors = ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"];
                let ecl = test.split( ":" );
                let color = ecl[1].replace( /\n/, "" );
                colors.indexOf( color) === -1 ? invalid++ : potential++;
            }
            if ( test.includes( "pid" ) ) {
                let x = 0;
                let pid = test.split( ":" );
                let el = pid[1].replace( /\n/, "" );
                if ( el.length === 9 ) {
                    x = (/[0-9]{9}/).test( el );
                }
                x ? potential++ : invalid++ ;
        ...
        potential < 7 ? invalid++ : null;

        invalid === 0 ? valid++ : null;
    }
    console.log( "Part Two Solution: " + valid );
}
